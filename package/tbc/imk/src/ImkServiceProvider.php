...
class ImkServiceProvider {
    ...
    function setImkKey($key) {
        $this->api_imk_key = $key;
        return $this;
    }
    ...
    function getBlogs($params = []) {
        ...
        if (isset($_GET['debug_api'])) {
            echo "URL IS: " . $blogurl . " <br>";
            $raw = $this->client->request('GET', $blogurl, [], ['raw' => true]);
            echo "RAW IS: " . $raw; exit;
        }
    ...
    function getusergroups() {
         // echo $this->api_url."api/v1/user-groups/?apiKey=".$this->api_imk_key ; die;
          $fp = $this->client->request('GET', "api/v1/user-groups/?apiKey=".$this->api_imk_key);
          if (isset($fp)) {
              return $fp;
          } else {
              return [];
