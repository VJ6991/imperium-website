...
class Client {
    ...
    function request($method, $url, $params = [], $options = []) {
        try {
            ...
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // Fix for local Windows development SSL errors
                    ...
                    if ($output = json_decode($result)) {
                        if (isset($options['withSuccess']) && $options['withSuccess'] == true) {
                            ...
                            if (isset($output->success)) {
                                if ($output->success) {
                                    return isset($output->data) ? $output->data : $output;
                                } else {
                                    // If success is false, sometimes it still has data, handle it
                                    return isset($output->data) ? $output->data : $output;
                                }
                            } else {
                                // Some endpoints (like blogs) return the data without a success wrapper
                                return $output;
                            }
                        }
                    } else {
