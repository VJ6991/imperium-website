...
class Controller extends ImkServiceProvider {
    ...
    protected function getLocalBlogs() {
        ...
        if (file_exists($blogsFile)) {
            ...
            if (is_array($data)) {
                foreach ($data as $item) {
                    ...
                    $img = $item['image'] ?? '';
                    // Keep an empty image empty so the view hides the featured
                    // image block. Running '' through url() yields "/", which
                    // renders as a broken image instead of being hidden.
                    if ($img === '') {
        ...
        // Latest first. Sort by date descending (ISO yyyy-mm-dd sorts lexically),
        // so ordering is correct regardless of the order entries sit in the file.
